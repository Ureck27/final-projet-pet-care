...
describe('Pet API', () => {
  ...
  describe('POST /api/pets', () => {
    it('should create a pet successfully', async () => {
      ...
      const res = await request(app).post('/api/pets').set('Cookie', userToken).send(petData);
  ...
  describe('GET /api/pets/user', () => {
    it('should get all pets for the logged-in user', async () => {
      ...
      const res = await request(app).get('/api/pets/user').set('Cookie', userToken);
  ...
  describe('DELETE /api/pets/:id', () => {
    it('should allow owner to delete their pet', async () => {
      ...
      const res = await request(app).delete(`/api/pets/${pet._id}`).set('Cookie', userToken);
    ...
    it('should prevent other users from deleting the pet', async () => {
      ...
      const res = await request(app).delete(`/api/pets/${pet._id}`).set('Cookie', otherToken);
