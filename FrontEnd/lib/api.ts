...
export const authApi = {
  ...
  uploadProfileImage: (image: File) => {
    ...
    return api.put<{ message: string; profileImage: string }>('/users/profile-image', formData);
  }
};
...
export const timelineApi = {
  createPost: (postData: FormData) =>
    api.post<TimelinePost>('/timeline', postData),
  
  getTimeline: (petId: string) =>
    api.get<TimelinePost[]>(`/timeline/${petId}`),
  
  deletePost: (id: string) =>
    api.delete<{ message: string }>(`/timeline/${id}`),
};
