...
export default function ProfilePage() {
  ...
  const handleSave = async () => {
    setIsSaving(true)
    // Here you would normally call an API to update user info
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (user && response.profileImage) {
        // This depends on how useAuth is implemented, but usually we'd want to update the user object
            ...
            <div className="flex flex-col items-center gap-6 md:flex-row md:items-start">
              <div className="relative">
                <Avatar className="h-32 w-32 border-4 border-primary/20">
                  <AvatarImage src={user.avatar || user.profileImage || "/placeholder.svg"} alt={user.fullName} />
