...
const PawPrintBackground = dynamic(
  () => import('@/components/ui/pet-illustrations').then((mod) => mod.PawPrintBackground),
  { ssr: false },
);
const FloatingPets = dynamic(
  () => import('@/components/ui/pet-illustrations').then((mod) => mod.FloatingPets),
  { ssr: false },
);
const AnimatedPawTrail = dynamic(
  () => import('@/components/ui/pet-illustrations').then((mod) => mod.AnimatedPawTrail),
  { ssr: false },
);
const PetCareIcons = dynamic(
  () => import('@/components/ui/pet-illustrations').then((mod) => mod.PetCareIcons),
  { ssr: false },
);
...
const HeroSlider = dynamic(
  () => import('@/components/layout/hero-slider').then((mod) => mod.HeroSlider),
  {
    ssr: false, // Ensure it's purely client side to avoid hydration mismatches with carousels
    loading: () => (
      <div className="w-full aspect-video md:aspect-[4/3] bg-muted/20 animate-pulse rounded-2xl" />
    ),
  },
);
  ...
  const coreFeatures = [
    {
      ...
      description:
        "Schedule and track walks, play, training, meals, and rest routines personalized to your pet's needs",
    },
    {
      icon: Users,
      title: 'Professional Care Booking',
      description:
        'Verified caregivers with credentials, certifications, and proven experience—not just random sitters',
    },
    {
      icon: Video,
      title: 'AI-Verified Timeline',
      description:
        "Real-time photo/video updates with emotion detection showing your pet's mood and activity verification",
    },
  ];
          ...
          <motion.div style={{ y: yHero }} className="container mx-auto px-4 relative z-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-start lg:items-center">
              ...
              <div className="flex flex-col items-start text-left max-w-2xl">
                <FadeIn direction="up">
                  <Badge className="mb-6 bg-primary/20 text-primary hover:bg-primary/25 border border-primary/30 px-4 py-2 text-sm shadow-soft-lg transition-smooth">
                ...
                <FadeIn direction="up" delay={0.1}>
                  <div
                    ref={heroRef}
                    className="mb-6 space-y-2 leading-tight text-shadow-sm float-element text-white dark:text-orange-300"
                  >
                    <SplitText
                    ...
                    <SplitText
                      ...
                      tag="h1"
                      className="block text-balance text-4xl md:text-5xl lg:text-6xl font-extrabold text-foreground md:text-transparent md:bg-clip-text md:bg-gradient-to-r md:from-primary md:to-secondary"
                      delay={50}
                      duration={1.2}
                      ease="easeOut"
                      splitType="chars"
                ...
                <FadeIn direction="up" delay={0.2}>
                  <p className="mb-8 text-pretty text-lg text-muted-foreground md:text-xl leading-relaxed">
                    All-in-one platform combining daily activity management, verified professional
                    caregivers, and AI-verified updates.
                ...
                <FadeIn direction="up" delay={0.3} className="w-full">
                  <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
                    ...
                    >
                      <Button
                        size="lg"
                        className="h-14 px-8 text-base bg-primary hover:bg-primary/90 text-primary-foreground shadow-soft-lg w-full sm:w-auto transition-smooth border-0"
                        asChild
                      >
                        <Link href="/register">
                          Find Your Caregiver
                    ...
                    >
                      <Button
                        size="lg"
                        className="h-14 px-8 text-base bg-background text-primary border-2 border-primary hover:bg-primary/5 w-full sm:w-auto transition-smooth shadow-soft-lg"
                        asChild
                      >
      ...
      <section className="px-4 py-20 relative paws-relax-diff">
        ...
        <div className="container mx-auto">
          <FadeIn direction="up" className="mb-16 text-center">
            <Badge
              variant="outline"
              className="mb-4 border-primary/30 bg-primary/5 text-primary font-semibold shadow-soft-lg hover:shadow-xl transition-smooth"
            >
              Three Problems. One Solution.
            </Badge>
            <h2 className="mb-4 text-4xl font-bold text-foreground">The Paws & Relax Difference</h2>
            ...
            {problemSolution.map((item) => (
              <StaggerItem key={item.number}>
                <TiltCard intensity={10}>
                  <Card className="feature-card h-full bg-card backdrop-blur-sm border border-border shadow-soft-lg hover:shadow-2xl hover:border-primary/50 transition-smooth overflow-hidden group">
                    <CardContent className="p-8 pb-10">
                      ...
                      >
                        {item.icon && <item.icon className="h-8 w-8 icon-visible text-white" />}
                      </motion.div>
                      <p className="mb-6 text-base font-medium text-foreground leading-relaxed">
                        {item.problem}
                      </p>
                      <div className="pt-6 border-t border-border bg-gradient-to-r from-primary/5 to-secondary/5 p-6 -mx-8 -mb-10 rounded-b-lg group-hover:from-primary/10 group-hover:to-secondary/10 transition-smooth">
                        <p className="text-base text-muted-foreground font-medium flex items-start gap-2">
                          <CheckCircle2 className="h-5 w-5 mt-0.5 flex-shrink-0 icon-visible text-primary" />
                          {item.solution}
                        </p>
                      </div>
                    </CardContent>
      ...
      <section id="features" ref={featuresRef} className="px-4 py-20 relative">
        <div className="container mx-auto">
          <FadeIn direction="down" className="mb-16 text-center">
            <Badge
              variant="outline"
              className="mb-4 border-primary/30 bg-primary/5 text-primary shadow-sm hover:shadow transition-smooth"
            >
              Core Services
            </Badge>
            <h2 className="mb-4 text-4xl font-bold text-gray-900 dark:text-foreground">
              Everything You Need
            </h2>
          ...
          >
            {howItWorks.map((step, index) => (
              <StaggerItem key={step.title} className="relative h-full flex flex-col">
                <Card className="h-full border-border shadow-soft dark:shadow-[0_8px_30px_rgba(0,0,0,0.3)] hover:shadow-xl hover:border-primary/50 hover:-translate-y-[6px] hover:scale-[1.02] transition-all duration-300 relative bg-card dark:hover:bg-card/80 group overflow-hidden flex flex-col rounded-[1.5rem]">
                  <motion.div
                    initial={{ scale: 0 }}
                    whileInView={{ scale: 1 }}
                    transition={{ delay: index * 0.2 + 0.3 }}
                    className="absolute -right-12 -top-12 w-40 h-40 bg-primary/10 dark:bg-primary/20 blur-xl rounded-full group-hover:scale-150 transition-transform duration-700 ease-out z-0"
                  />

                  {/* Fixed Step Badge Top Right */}
                  <div className="absolute top-6 right-6 z-20 flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-primary to-primary/80 text-primary-foreground text-lg font-bold shadow-soft group-hover:scale-110 transition-all duration-300 cursor-default">
                    {index + 1}
                  </div>

                  <CardContent className="p-8 md:p-10 relative z-10 flex flex-col flex-1">
                    <div className="mb-8 mt-2">
                      <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-primary/10 group-hover:bg-gradient-to-br group-hover:from-primary group-hover:to-primary/80 group-hover:text-primary-foreground transition-all duration-300 shadow-sm border border-transparent dark:border-primary/20 text-primary">
                        {step.icon && (
                          <step.icon className="h-8 w-8 dark:drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)] transition-colors" />
                        )}
                      </div>
                    </div>
                    <h3 className="mb-4 text-xl md:text-2xl font-bold text-foreground group-hover:text-primary transition-colors tracking-tight">
                      {step.title}
                    </h3>
                  ...
                  <Card className="border-border shadow-soft hover:shadow-xl hover:border-primary/50 transition-smooth overflow-hidden bg-glass h-full flex flex-col group">
                    ...
                    <CardContent className="p-6 flex-1 flex flex-col">
                      ...
                      <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                        <Button
                          className="w-full mt-6 shadow-soft hover:shadow-md transition-smooth"
                          variant="outline"
                          asChild
                        >
      ...
      <section className="px-4 py-16">
        <div className="container mx-auto">
          <div className="mb-12 text-center">
            <h2 className="mb-4 text-3xl font-bold text-primary">Your Pet&apos;s Magical Day</h2>
            <p className="mx-auto max-w-2xl text-muted-foreground">
              See real proof of every walk, cuddle, snack, and nap—brought to life with playful
              cards and motion.
            </p>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            <Card className="border-border shadow-soft bg-glass">
              <CardContent className="p-6">
                ...
                <p className="text-sm text-muted-foreground">
                  Verified credentials, certifications, background checks, and documented
                  experience—not just animal lovers.
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="border-border shadow-soft bg-glass">
              <CardContent className="p-6">
                ...
                <p className="text-sm text-muted-foreground">
                  Personalized care plans, scheduled activities, health monitoring, and enrichment
                  programs tailored to your pet's needs.
                </p>
              </CardContent>
            </Card>
            ...
            <Card className="border-border shadow-soft bg-glass">
              <CardContent className="p-6">
                ...
                <p className="text-sm text-muted-foreground">
                  Emotion detection, activity logging, mood tracking, and transparent timeline—proof
                  that care actually happened.
                </p>
              </CardContent>
            </Card>
          </div>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            {testimonials.map((testimonial) => (
              <Card
                ...
                className="border-border shadow-soft hover:shadow-md transition-shadow bg-glass"
              >
                <CardContent className="p-6">
            ...
            {[
              {
                ...
                desc: 'Activity management + booking + tracking',
              },
              {
                icon: CheckSquare,
                title: 'Verified Proof',
                desc: 'AI-verified updates, not just promises',
              },
              { icon: Target, title: 'Unified Platform', desc: 'Replace 5+ services with one app' },
              { icon: Shield, title: 'Trusted Network', desc: '10,000+ happy pets and counting' },
            ].map((item, index) => (
              <Card
                key={`${item.title}-${index}`}
                className="border-border shadow-soft text-center p-6 bg-glass transition-smooth hover:shadow-md hover:border-primary/30"
              >
      ...
      <section className="relative overflow-hidden bg-gradient-to-r from-primary via-primary/95 to-secondary px-4 py-20">
        ...
        <div className="container relative z-10 mx-auto text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <Button
              size="lg"
              className="shadow-soft bg-secondary hover:bg-secondary/90 text-foreground"
              asChild
            >
...
const TimelineGridItem = ({ icon, title, description }: Omit<TimelineGridItemProps, 'area'>) => {
  return (
