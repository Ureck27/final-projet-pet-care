                      ...
                      <TableCell>
                        <div className="flex flex-col gap-1">
                          {request.profileImage && (
                            <a 
                              href={`${process.env.NEXT_PUBLIC_API_URL || ''}${request.profileImage}`} 
                              target="_blank" 
                              rel="noopener noreferrer"
                              className="text-xs text-blue-600 hover:underline flex items-center gap-1"
                            >
                              <Eye className="h-3 w-3" /> Profile Photo
                            </a>
                          )}
                          {request.certificateImages && request.certificateImages.map((cert, idx) => (
                            <a 
                              key={idx}
                              href={`${process.env.NEXT_PUBLIC_API_URL || ''}${cert}`} 
                              target="_blank" 
                              rel="noopener noreferrer"
                              className="text-xs text-blue-600 hover:underline flex items-center gap-1"
                            >
                              <Eye className="h-3 w-3" /> Cert {idx + 1}
                            </a>
                          ))}
                          {request.idCardImage && (
                            <a 
                              href={`${process.env.NEXT_PUBLIC_API_URL || ''}${request.idCardImage}`} 
                              target="_blank" 
                              rel="noopener noreferrer"
                              className="text-xs text-blue-600 hover:underline flex items-center gap-1"
                            >
